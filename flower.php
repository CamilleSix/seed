...
<article>
	<section class="total">
		...
		$nombreFeuilleDef = 8 ;
		$nombreFleurDef = 8 ;
				$tailleMoyenneTige = rand(15,20); // 1 très courtes tiges, 15 très grandes tiges
				$nombreDivisionBranche = rand(2,5);
				$nTotalBranche = rand(1,3);

				$largeurBaseBranche = rand(50,100) / 100  ;
				$quantiteFleur = 0 + rand(30,230) / 100 ;

				$tigeFleur = 1 ; // distance de la "tige" potentielle qui relie la Branche à la fleur, 0 si "pas de tige"
...
// Tige
				...
				for ($i=0; $i < $nTotalBranche; $i++) { 
					...
					$start['y'] = 91 ;
					$step = 1 ;
					$finish['x'] = rand(-8,8).'.'.rand(1,99); 
					$finish['y'] = rand(-20,-40).'.'.rand(1,99); 
					...
					if (!empty($arc['x'])){
						...
						$arc['x'] =  $finish['x'] + rand(-100,100) / 10;
					}

					$arc['y'] = $finish['y'] + rand(-100,200) / 10; 
					...
					q '.$arc['x'].' '.$arc['y'].' '.$finishX.','.$finishY.'" fill ="none" stroke ="rgb('.$colorBranche[1].','.$colorBranche[2].','.$colorBranche[3].')" stroke-width ="'.$largeurBaseBrancheTige.'"/>';
/*
					...
					$svg .= '<circle cx="'.$finalX.'" cy="'.$finalY.'" r="'.($largeurBaseBrancheTige / 2).'" fill="rgb('.$colorBranche[1].','.$colorBranche[2].','.$colorBranche[3].')"/>';
...
*/
					...
					$saveY = $startY;
						...
						// pour les fleurs toutes les tiges doivent partir de l'origine
				...
				// le nombre de feuille dépend de la taille maximale (hauteur) de la feuille
					for ($f=0; $f < $nFeuille ; $f++) { 
						...
						if (rand(1,2) == 1){
							$decaleX = $startX  + rand(0,99)/100 ;
							$decaleY = $startY  + rand(0,99)/100 ;
						} else {
							$decaleX = $startX ;
							$decaleY = $startY ;
						}
				...
				// le nombre de fleur dépend de la taille maximale (hauteur) de la feuille
					for ($f=0; $f < $nFleur; $f++) { 
						...
						x="'.($finalX).'" y="'.($finalY - $tailleMaxFleur).'" transform="rotate('.rand(90,180).' '.($finalX).' '.($finalY ).')" />';
