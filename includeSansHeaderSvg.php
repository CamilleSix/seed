...
function getColorRandom($step = 1, $class = 'baseColor', $modifHex = 10){
...
function randParam($param){
	...
	return $rand ;
		...
		$nombreFeuilleDef = 8 ;
		$nombreFleurDef = 8 ;
				$tailleMoyenneTige = 7; // 1 très courtes tiges, 15 très grandes tiges
				$nombreDivisionBranche = rand(1,8);
				$nTotalBranche = rand(10,40) ;

				$largeurBaseBranche = rand(100,200) / 100  ;

				$shapeFeuille = 0 + (rand(20, 90) / 90) ;
				$shapeFleur = 0 + (rand(20, 90) / 90) ;
...
/*
				...
				$colorTige = getColorRandom(3, 'colorBranche', 2) ;
				...
				for ($i=1; $i <= $nombreFleurDef ; $i++) { 
					...
					$colorCentre = getColorRandom(1, 'colorFleur2', 2) ;
					$colorCentreSmall = getColorRandom(5, 'colorFleur2', 5) ;
...
// Tige
					...
					for ($tour=0; $tour <= $nPetale ; $tour++) { 
						...
						$color = getColorRandom($tour, 'colorFleur1', -5) ;
				...
				$minTige = (-5 - $tailleMoyenneTige);
				$maxTige = (5 + $tailleMoyenneTige)  ;
				$growState = 1.2 ;
				// 1 taille max
			...
			// réduit de 10 tons le vert à la fin de chaque branche si rangée suivante
						$finish['x'] = rand($minTige,$maxTige).'.'.rand(1,99); 
					...
					$colorBranche = getColorRandom($step, 'colorBranche', 1) ;
					$startX = $start['x'] ; 
					$startY = $start['y'] ; 
					...
					$svg .= '<circle cx="'.$finalX.'" cy="'.$finalY.'" r="'.($largeurBaseBrancheTige / 2).'" stroke="rgb('.($colorBranche[1] + 5).','.($colorBranche[2] + 5).','.($colorBranche[3] + 5).')" fill="rgb('.$colorBranche[1].','.$colorBranche[2].','.$colorBranche[3].')"/>';
				...
				file_put_contents("svg".rand(1,15).".svg", $svg) ;
